    ...
    <div className="sticky bottom-0 z-20 -mx-4 border-t bg-surface/95 px-4 py-3 shadow-raised backdrop-blur sm:-mx-6 sm:px-6">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-wrap items-center gap-x-5 gap-y-3">
          {/* The total is a fact about what was written down and stays. The
              ring and the gap beside it were a target, and a target on the
              same screen as the input is an instruction to close it. */}
          {showTargets ? (
            <ProgressRing value={totals.total} target={totals.expected} />
          ) : (
            <Stat label="Logged" value={formatHours(totals.total)} />
          )}
